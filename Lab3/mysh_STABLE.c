/************************************************************************\
			Lab 3: sh Simulator
Engineer: Nathan VelaBorja
Date: Feb. 16, 2016
Description: This program simulates the Unix sh for command processing. 
Version: 1.0
	...
	else							// What child process will do
	{
	        ...
	        if (hasPipe(line))				// If input is piped, do a seperate fork and pipe process
	        {
        	        ...
        	        if (pid)        			// parent as pipe writer
        	        {
        	                close(1);
        	                dup(pd[1]);
        	                execve(binPath, headArgs, env);
      		      	}
                	else            			// child as pipe reader
                	{
                	        close(0);
                	        dup(pd[0]);
                	        execve(binPath2, tailArgs, env);
               		}
			return 1;
		}
		...
		ret = execve(binPath, arguments, env);		// Execute command
		exit(1);
	}
...
int printBanner(void)
{
	printf("******** VelaBorja's Single Command I/O Redirection Console *********\n\n");
