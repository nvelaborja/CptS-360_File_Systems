		...
		if (isServerCommand(command))
		{
			n = write(server_sock, line, MAX);		// Send input line to server
				...
				char *buff[MAX];		// Buffer for file writing 
                                ...
                                n = read(server_sock, &fileSize, sizeof(fileSize));	// Read the file size from server
				...
				newFile = open(arguments[0], O_WRONLY | O_CREAT, 0644);	// Open new file for writing				
				while (count < fileSize)
				{
					n = read(server_sock, buff, MAX);		// Read into buffer from server
					printf("Received %d bytes from server.\n", n);
					count += n;					// Updated byte counter
					write(newFile, buff, MAX);			// Write buffer to file
					bzero(buff, MAX);
					buff[MAX-1] = '\0';
				}				

				fclose(newFile);					// Close file when we're all done
