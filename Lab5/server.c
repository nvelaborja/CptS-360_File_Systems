...
int myget(void)
{
	...
	if ( (r = lstat(filePath, &fstat)) < 0)	// If stat returned with an error
	{
		int error = -1;
		printf("Couldn't stat given file path.\n");
		write(client_sock, &error, sizeof(error)); 
		return;
	}

	// At this point, the stat struct has been successfully loaded

	fileSize = (int)sp->st_size;		// Record the file's size
printf("File Size Sending to Client: %d\n", fileSize);
	write(client_sock, &fileSize, sizeof(fileSize));	// Send the filesize to the client

	file = fopen(filePath, "r");		// Open file for reading
	if (file) printf("File successfully opened for read.\n");
printf("File opened\n");
	while ((n += read(file, fileBuff, MAX)) && n < fileSize)
	{
printf("Writing something to client\n");
		write(client_sock, fileBuff, MAX);	// Write file contents to client
	}

	fclose(file);				// Close file when we're all done
printf("File Closed\n");
}
